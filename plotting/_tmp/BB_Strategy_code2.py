...
ticker['sma_20'] = sma(ticker['close'], 20)
print(ticker.tail())

ticker['close'].plot(label = 'CLOSE', alpha = 0.6)
ticker['sma_20'].plot(label = 'SMA 20', linewidth = 2)
plt.xlabel('Date')
plt.ylabel('Closing Prices')
...
ticker['upper_bb'], ticker['lower_bb'] = bb(ticker['close'], ticker['sma_20'], 20)
print(ticker.tail())

ticker['close'].plot(label = 'CLOSE PRICES', color = 'skyblue')
ticker['upper_bb'].plot(label = 'UPPER BB 20', linestyle = '--', linewidth = 1, color = 'black')
ticker['sma_20'].plot(label = 'MIDDLE BB 20', linestyle = '--', linewidth = 1.2, color = 'grey')
ticker['lower_bb'].plot(label = 'LOWER BB 20', linestyle = '--', linewidth = 1, color = 'black')
plt.legend(loc = 'upper left')
...
buy_price, sell_price, bb_signal = implement_bb_strategy(ticker['close'], ticker['lower_bb'], ticker['upper_bb'])

ticker['close'].plot(label = 'CLOSE PRICES', alpha = 0.3)
ticker['upper_bb'].plot(label = 'UPPER BB', linestyle = '--', linewidth = 1, color = 'black')
ticker['sma_20'].plot(label = 'MIDDLE BB', linestyle = '--', linewidth = 1.2, color = 'grey')
ticker['lower_bb'].plot(label = 'LOWER BB', linestyle = '--', linewidth = 1, color = 'black')
plt.scatter(ticker.index, buy_price, marker = '^', color = 'green', label = 'BUY', s = 200)
plt.scatter(ticker.index, sell_price, marker = 'v', color = 'red', label = 'SELL', s = 200)
...
bb_signal = pd.DataFrame(bb_signal).rename(columns = {0:'bb_signal'}).set_index(ticker.index)
position = pd.DataFrame(position).rename(columns = {0:'bb_position'}).set_index(ticker.index)
...
for i in range(len(ticker_ret)):
    try:
        returns = ticker_ret['returns'][i]*strategy['bb_position'][i]
...
benchmark = get_benchmark(ticker['close'], '2020-01-01', 100000)
