...
#data['EMA_144'] = data['close'].ewm(span=144, min_periods=144, adjust=False).mean()
#data['EMA_169'] = data['close'].ewm(span=169, min_periods=169, adjust=False).mean()
...
if (close.iloc[-1] > sma_5.iloc[-1]) and (ema_144.iloc[-1] > ema_169.iloc[-1]):
    print_log ( '71_EMA_SMA', 'LONG', [ 'EMA_144', 'EMA_169', 'SMA_5' ] )
...
if (close.iloc[-1] < sma_5.iloc[-1]) and (ema_169.iloc[-1] > ema_144.iloc[-1]):
    print_log ( '71_EMA_SMA', 'SHORT', [ 'EMA_144', 'EMA_169', 'SMA_5' ] )
