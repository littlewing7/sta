...
def get_historic_data(symbol):
    return yf.download(symbol, start='2020-01-01', end='2023-04-19').drop('Adj Close', axis=1).drop('Volume', axis=1)
...
plt.plot(msft['Close'], alpha=0.3, label='BAYN.DE')
...
plt.legend(loc='upper left')
plt.title('BAYER WMA CROSSOVER TRADING SIGNALS')
plt.show()
