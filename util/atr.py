...
def __ATR3 (df, n):
    ...
    TR_l = [0]
    while i < df.index[-1]:
        TR = max(df['High'].at[i+1], df['Adj Close'].at[i]) - min(df['Low'].at[i+1], df['Adj Close'].at[i])
        TR_l.append(TR)
        i = i + 1
    TR_s = pd.Series(TR_l)
...
def calculate_ATR(df_func):
    ...
    high_close = np.abs(df_func['High'] - df_func['Adj Close'].shift())
    low_close = np.abs(df_func['Low'] - df_func['Adj Close'].shift())
